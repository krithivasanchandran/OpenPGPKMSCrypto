...
public class SecKeyRing {

	protected List<PGPSecretKeyRing> keys = new ArrayList<>();

	public PGPSecretKey getPGPSecretKeyRing(String userId) {

		for (PGPSecretKeyRing ring : keys) {

			PGPSecretKey secretKey = ring.getSecretKey();

			while (ring.getSecretKey().getUserIDs().hasNext()) {

				if (ring.getSecretKey().getUserIDs().next().toString()
						.contains(userId)) {
					return secretKey;
				}

				System.out.println(
						ring.getSecretKey().getUserIDs().next().toString());
	 ...
	 */
	public SecKeyRing(String armor) throws IOException, PGPException {
	 ...
	 */
	public SecKeyRing(InputStream stream) throws IOException, PGPException {
	 ...
	 */
	public List<PGPSecretKeyRing> load(String armor)
	 ...
	 * ring's existing list of keys.
	 */
	public List<PGPSecretKeyRing> load(InputStream stream)
			throws IOException, PGPException {
		ArrayList<PGPSecretKeyRing> keys = new ArrayList<PGPSecretKeyRing>();
		...
		while (packets.hasNext()) {
			...
			if (packet instanceof PGPSecretKeyRing)
				keys.add((PGPSecretKeyRing) packet);
